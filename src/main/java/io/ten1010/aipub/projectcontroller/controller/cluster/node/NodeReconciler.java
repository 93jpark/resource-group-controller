...
public class NodeReconciler implements Reconciler {
    ...
    private static final String MSG_NODE_BELONGS_TO_MULTIPLE_NODE_GROUPS = "Node [%s] belongs to multiple node groups";
    ...
    /**
     * Check whether given taint has NodeGroup{@link V1alpha1NodeGroup} exclusive key.
     * <p>
     * See constant string in TaintConstants{@link Taints}.
     * </p>
     *
     * @param taint
     * @return {@code true} if this taint has NodeGroup{@link V1alpha1NodeGroup} exclusive key
     */
    private static boolean isNodeGroupTaint(V1Taint taint) {
        if (taint.getKey() == null) {
            return false;
        }

        return taint.getKey().equals(Taints.KEY_NODE_GROUP);
    }

    /**
     * Build exclusive taints for NodeGroup{@link V1alpha1NodeGroup} which has no schedule and no execute effect, and has resource group name as value.
     *
     * @param group
     * @return a list of taints
     */
    private static List<V1Taint> buildNodeGroupTaints(V1alpha1NodeGroup group) {
    ...
    public NodeReconciler(Indexer<V1Node> nodeIndexer, Indexer<V1alpha1NodeGroup> groupIndexer, CoreV1Api coreV1Api, EventRecorder eventRecorder) {
            ...
            if (groups.size() > 1) {
                for (V1alpha1NodeGroup g : groups) {
                    this.eventRecorder.event(
                            g,
                            EventType.Warning,
                            Events.REASON_NODE_CONFLICT, MSG_NODE_BELONGS_TO_MULTIPLE_NODE_GROUPS,
     ...
     */
    private Map<String, String> reconcileLabels(V1Node node, @Nullable V1alpha1NodeGroup group) {
        Map<String, String> labels = getLabels(node);
        labels.remove(Labels.KEY_NODE_GROUP);
        if (group == null) {
            return labels;
        }
        labels.put(Labels.KEY_NODE_GROUP, K8sObjectUtil.getName(group));
        return labels;
    }
     ...
     */
    private List<V1Taint> reconcileTaints(V1Node node, @Nullable V1alpha1NodeGroup group) {
        List<V1Taint> taints = getTaints(node).stream()
                .filter(e -> !isNodeGroupTaint(e))
                .collect(Collectors.toList());
        if (group == null) {
            return taints;
        }
        taints.addAll(buildNodeGroupTaints(group));
        return taints;
    }
    ...
    private void updateNode(V1Node target, Map<String, String> labels, List<V1Taint> taints) throws ApiException {
        V1Node updated = new V1NodeBuilder(target)
                ...
                .build();
        this.coreV1Api
                .replaceNode(K8sObjectUtil.getName(target), updated)
                .execute();
    }
