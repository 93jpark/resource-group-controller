...
public class ImageNamespaceGroupBindingWatch implements ControllerWatch<V1alpha1ImageNamespaceGroupBinding> {
    ...
    public static class EventHandler implements ResourceEventHandler<V1alpha1ImageNamespaceGroupBinding> {
        ...
        public EventHandler(WorkQueue<Request> queue, Indexer<V1ReplicaSet> replicaSetIndexer, Indexer<V1alpha1Project> projectIndexer) {
            this.queue = queue;
        ...
        public void onAdd(V1alpha1ImageNamespaceGroupBinding obj) {
            obj.getProjects().stream()
                    ...
                    .flatMap(project ->
                            Optional.ofNullable(resolveToReplicaSet(project.getNamespace()))
        ...
        public void onUpdate(V1alpha1ImageNamespaceGroupBinding oldObj, V1alpha1ImageNamespaceGroupBinding newObj) {
            if (!oldObj.getProjects().equals(newObj.getProjects())) {
                getAddedOrDeletedProjects(oldObj.getProjects(), newObj.getProjects())
                        ...
                        .map(project -> this.resolveToReplicaSet(project.getNamespace()))
                        .forEach(replicaSets -> {
                            replicaSets.stream()
                                    .map(EventHandlerUtil::buildRequestFromNamespacedObject)
        ...
        private List<V1ReplicaSet> resolveToReplicaSet(String namespaceName) {
            return this.replicaSetIndexer.byIndex(IndexNames.BY_NAMESPACE_NAME_TO_REPLICA_SET_OBJECT, namespaceName);
        }
    ...
    public ImageNamespaceGroupBindingWatch(WorkQueue<Request> queue, Indexer<V1ReplicaSet> replicaSetIndexer, Indexer<V1alpha1Project> projectIndexer) {
        this.queue = queue;
        this.replicaSetIndexer = replicaSetIndexer;
        this.projectIndexer = projectIndexer;
    }
    ...
    public ResourceEventHandler<V1alpha1ImageNamespaceGroupBinding> getResourceEventHandler() {
        return new EventHandler(this.queue, this.replicaSetIndexer, this.projectIndexer);
    }
