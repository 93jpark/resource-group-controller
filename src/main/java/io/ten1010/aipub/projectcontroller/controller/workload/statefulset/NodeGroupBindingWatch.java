...
public class NodeGroupBindingWatch implements ControllerWatch<V1alpha1NodeGroupBinding> {
    ...
    public static class EventHandler implements ResourceEventHandler<V1alpha1NodeGroupBinding> {
        ...
        public EventHandler(WorkQueue<Request> queue, Indexer<V1StatefulSet> statefulSetIndexer, Indexer<V1alpha1Project> projectIndexer) {
            this.queue = queue;
        ...
        public void onAdd(V1alpha1NodeGroupBinding obj) {
            obj.getProjects().stream()
                    ...
                    .flatMap(project ->
                            Optional.of(resolveToStatefulSet(project.getNamespace()))
        ...
        public void onUpdate(V1alpha1NodeGroupBinding oldObj, V1alpha1NodeGroupBinding newObj) {
            if (!oldObj.getProjects().equals(newObj.getProjects())) {
                getAddedOrDeletedProjects(oldObj.getProjects(), newObj.getProjects())
                        ...
                        .map(project -> resolveToStatefulSet(Objects.requireNonNull(project.getNamespace())))
                        .forEach(statefulSets -> {
                            statefulSets.stream()
                                    .map(EventHandlerUtil::buildRequestFromNamespacedObject)
                                    .forEach(this.queue::add);
                        });
        ...
        private List<V1StatefulSet> resolveToStatefulSet(String namespaceName) {
            Objects.requireNonNull(namespaceName);
            return this.statefulSetIndexer.byIndex(IndexNames.BY_NAMESPACE_NAME_TO_STATEFUL_SET_OBJECT, namespaceName);
        }
    ...
    public NodeGroupBindingWatch(WorkQueue<Request> queue, Indexer<V1StatefulSet> statefulSetIndexer, Indexer<V1alpha1Project> projectIndexer) {
        this.queue = queue;
        this.statefulSetIndexer = statefulSetIndexer;
        this.projectIndexer = projectIndexer;
    }
    ...
    public ResourceEventHandler<V1alpha1NodeGroupBinding> getResourceEventHandler() {
        return new EventHandler(this.queue, this.statefulSetIndexer, this.projectIndexer);
    }
